...
namespace wSelect
{
	...
	public partial class BoxDetails : UserControl
	{
		...
		public void PopulateDelList(int prmPolicyNo)
		{
			lstImageDel.Items.Clear();
			CtrlPolicy ctrlPolicy = new CtrlPolicy(pBox.ctrlBox.ProjectCode,pBox.ctrlBox.BatchKey,pBox.ctrlBox.BoxNumber,prmPolicyNo);
		    wItem policy = new wfePolicy(sqlCon, ctrlPolicy);
...
//            policyState[0]=currState;
		...
		{
            CtrlPolicy ctrlPolicy = new CtrlPolicy(pBox.ctrlBox.ProjectCode,pBox.ctrlBox.BatchKey,pBox.ctrlBox.BoxNumber,0);
		    wfePolicy policy = new wfePolicy(sqlCon, ctrlPolicy);
            lblNRCNo.Text = policy.GetPolicyKeyNo(Convert.ToInt32(lstPolicy.SelectedItem));
			PopulateImageList(Convert.ToInt32(lstPolicy.SelectedItem));
       ...
       private void ClearSelection()
		...
		public bool DeleteNotify(int currIndex)
		{
			...
			PopulateDelList(Convert.ToInt32(lstPolicy.SelectedItem.ToString()));
//			//MoveNext();
//			lstImage.Refresh();
//			lstImageDel.Refresh();
			return true;
