...
namespace ImageHeaven
{
	...
	public partial class aeFQC : Form, IMessageFilter ,StateData
	{
		...
		void AeFQCLoad(object sender, EventArgs e)
		{
            ...
            //}
            DisplayName();
            //txtRegional.GotFocus +=new EventHandler(txtRegional_GotFocus);
            txtRegional.Click +=new EventHandler(txtRegional_Click);
		}

        void txtRegional_Click(object sender, EventArgs e)
        {
            Process[] processes = Process.GetProcessesByName("PramukhIME");

            //Main part
            foreach (Process p in processes)
                if (p.ProcessName == (string)"PramukhIME")
                {
                    PostMessage(p.MainWindowHandle, WM_KEYDOWN, (int)Keys.F8, 0);
                }
        }
...
//        }
        ...
        private void DisplayName()
        {
            ...
            this.txtRegional.AutoCompleteCustomSource = wPolicy.Search(keyno.Text);
