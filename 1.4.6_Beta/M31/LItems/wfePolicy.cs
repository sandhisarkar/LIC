...
namespace LItems
{
	...
	public class wfePolicy: wItem, StateData
	{
        ...
        public DataSet GetAllPolicyDetails(wfeBox wBox,out OdbcDataAdapter pAdp)
        {
            ...
            try
            {
                sqlStr = "select Scan_upload_flag,date_format(scanned_date,'%Y%m%d') as scanned_date,Incremented_Scan,status,policy_number,box_number,proj_key,batch_key,key_no,Export_path from policy_master where proj_key=" + wBox.ctrlBox.ProjectCode + " and batch_key=" + wBox.ctrlBox.BatchKey + " order by box_number,policy_number";
                sqlAdap = new OdbcDataAdapter(sqlStr, sqlCon);
                sqlAdap.Fill(policyDs);
		...
		public DataSet GetPolicyList(eSTATES[] prmState)
		{
			...
			try 
			{
                string server = sqlCon.DataSource.Replace(" via TCP/IP", "");
                string conStr = "server=" + server + ";uid=root;pwd=root;database=ih_db_nrc;Charset=utf8;";
                ...
                {
                    //sqlStr = "select A.policy_no,A.name_of_policyholder,A.date_of_birth,A.date_of_commencement,B.status,B.count_of_pages,B.photo,b.key_no,c.unicode_name from rawdata A, policy_master B,tbl_unicode_text c where A.proj_key=B.proj_key and A.batch_key=B.batch_key and A.policy_no = B.policy_number and b.key_no = c.key_no and B.proj_key=" + ctrlPolicy.ProjectKey + " and B.batch_key=" + ctrlPolicy.BatchKey + " and B.box_number=" + ctrlPolicy.BoxNumber;
                    sqlStr = "select A.policy_no,A.name_of_policyholder,A.date_of_birth,A.date_of_commencement,B.status,B.count_of_pages,B.photo," +
                             " B.status,b.key_no,c.unicode_name from tbl_unicode_text c right join (policy_master B inner join rawdata A on A.proj_key=B.proj_key and A.batch_key=B.batch_key " +
                            " and A.policy_no = B.policy_number " +
                            " and B.proj_key=" + ctrlPolicy.ProjectKey + " and B.batch_key=" + ctrlPolicy.BatchKey + " and B.box_number=" + ctrlPolicy.BoxNumber + " ) on b.key_no = c.key_no";
                }
                else
                {
                    if (ctrlPolicy.BoxNumber != 0)
                    {
                        sqlStr = "select A.policy_no,A.name_of_policyholder,A.date_of_birth,A.date_of_commencement,B.status,B.count_of_pages,B.photo," +
                             " B.status,b.key_no,c.unicode_name from tbl_unicode_text c right join (policy_master B inner join rawdata A on A.proj_key=B.proj_key and A.batch_key=B.batch_key " +
                            " and A.policy_no = B.policy_number " +
                            " and B.proj_key=" + ctrlPolicy.ProjectKey + " and B.batch_key=" + ctrlPolicy.BatchKey + " and B.box_number=" + ctrlPolicy.BoxNumber;
                    }
                    else
                    {
                        sqlStr = "select A.policy_no,A.name_of_policyholder,A.date_of_birth,A.date_of_commencement,B.status,B.count_of_pages,B.photo,B.status,B.box_number,b.key_no,c.unicode_name from rawdata A, policy_master B,tbl_unicode_text c where A.proj_key=B.proj_key and A.batch_key=B.batch_key and A.policy_no = B.policy_number and b.key_no = c.key_no  and B.proj_key=" + ctrlPolicy.ProjectKey + " and B.batch_key=" + ctrlPolicy.BatchKey;
                    }
                    ...
                    sqlStr = sqlStr + ")) on b.key_no = c.key_no";
                }

				MySqlDataAdapter sqlAdap=new MySqlDataAdapter(sqlStr,con);
				sqlAdap.Fill(policyDs);
                con.Close();
			}
			catch (Exception ex) 
...
//		
        ...
        public int SearchForTransaction()
        {
            string sqlStr = null;
            DataSet policyDs = new DataSet();
            ...
            try
            {
                sqlStr = "select * from transaction_log" +
                 " where proj_key=" + ctrlPolicy.ProjectKey +
                " and batch_key=" + ctrlPolicy.BatchKey + " and box_number=" + ctrlPolicy.BoxNumber + " and policy_number = " + ctrlPolicy.PolicyNumber;
                sqlAdap = new OdbcDataAdapter(sqlStr, sqlCon);
                sqlAdap.Fill(policyDs);
...
public bool UpdateStatus(int pStatus,string pExportPath, Credentials prmCrd)
{
    string sqlStr = null;
    OdbcTransaction sqlTrans = null;
    bool commitBol = true;
    ...
    sqlStr = @"update policy_master" +
        " set status=" + pStatus + ",Export_path='"+pExportPath+"',modified_by='" + prmCrd.created_by + "',modified_dttm='" + prmCrd.created_dttm + "' where proj_key=" + ctrlPolicy.ProjectKey +
        " and batch_key=" + ctrlPolicy.BatchKey + " and box_number=" + ctrlPolicy.BoxNumber +
        " and policy_number=" + ctrlPolicy.PolicyNumber + " and status<>" + (int)eSTATES.POLICY_EXPORTED;
    ...
    return commitBol;
        ...
        public bool UpdateAllPolicyStatus(eSTATES state, Credentials prmCrd, OdbcTransaction prmTrans)
        {
            string sqlStr = null;
            OdbcTransaction sqlTrans = null;
            bool commitBol = true;
        ...
        public bool UpdateRegionalName(string prmEnglishName,string prmName,string keyNo)
        {
            string sqlStr = null;
            OdbcTransaction sqlTrans = null;
            bool commitBol = true;
            ...
            string englishName = prmEnglishName.TrimStart().TrimEnd();

            sqlStr = "insert into tbl_unicode_text values('"+ keyNo +"','"+ prmName +"','"+ englishName +"')";
                

            try
            {
                command.Connection = con;
                command.CommandText = "Delete from tbl_unicode_text where key_no='"+ keyNo +"'";
                command.ExecuteNonQuery();
        ...
        public string Search(string pKeyNo,string pNameLike)
        {
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                string sql = "select * from tbl_unicode_text where key_no='"+ pKeyNo +"'";
                ...
                else
                {
                    ...
                    {
                        ds = new DataSet();
                        sql = "select english_name,UNICODE_NAME,count(*) AS MAX_OCCR from tbl_unicode_text  where english_name='" + pNameLike + "' " +
                              " group by unicode_name order by max_occr desc limit 1";
        ...
        public AutoCompleteStringCollection  Search(string pEngName)
        {
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                string sql = "select distinct unicode_name from tbl_unicode_text where english_name like '%" + pEngName + "'";
                MySqlDataAdapter da = new MySqlDataAdapter(sql, con);
                da.Fill(ds);
                if (ds.Tables[0].Rows.Count > 0)
                {
        ...
        public string GetPolicyKeyNo(int pPolicyNo)
        {
            string sqlStr = null;
            ...
            try
            {
                sqlStr = "select key_no from policy_master where policy_number="+ pPolicyNo;
                sqlAdap = new OdbcDataAdapter(sqlStr, sqlCon);
                sqlAdap.Fill(expDs);
            }
            catch (Exception ex)
