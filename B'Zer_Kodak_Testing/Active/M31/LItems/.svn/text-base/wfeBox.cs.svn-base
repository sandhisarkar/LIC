...
namespace LItems
{
	...
	public class wfeBox: wItem, StateData
	{
		...
		public DataSet GetBox(eSTATES[] state,int prmBatchKey)
		{
			string sqlStr=null;
			DataSet dsBox=new DataSet();
			OdbcDataAdapter sqlAdap=null;
			
			sqlStr="select distinct box_number,count(policy_number) as policy_number from policy_master where trim(batch_key)=" + prmBatchKey ;
        ...
        public DataSet GetScannedBox(int prmBatchKey)
        {
            string sqlStr = null;
            DataSet dsBox = new DataSet();
            OdbcDataAdapter sqlAdap = null;

            sqlStr = "select distinct box_number from box_master  where status=10 and trim(batch_key)=" + prmBatchKey;
            try
            {
                sqlAdap = new OdbcDataAdapter(sqlStr, sqlCon);
                sqlAdap.Fill(dsBox);
