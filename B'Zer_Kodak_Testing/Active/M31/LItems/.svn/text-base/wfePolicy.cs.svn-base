...
namespace LItems
{
	...
	public class wfePolicy: wItem, StateData
	{
        ...
        public DataSet GetAllPolicyDetailsRaw(wfeBox wBox)
        {
            ...
            try
            {
                sqlStr = "select A.name_of_policyholder,A.date_of_birth,A.date_of_commencement,A.customer_id,A.policy_no,A.serial_number,A.batch_serial from rawdata A where A.proj_key=" + wBox.ctrlBox.ProjectCode + " and A.batch_key=" + wBox.ctrlBox.BatchKey;
                sqlAdap = new OdbcDataAdapter(sqlStr, sqlCon);
                sqlAdap.Fill(policyDs);
            }
            catch (Exception ex)
        ...
        public bool UpdateTransactionLog(eSTATES state,Credentials prmCrd)
        {
            ...
            switch (state)
            {
                case eSTATES.POLICY_SCANNED:
                    {
                        sqlStr = @"insert into transaction_log (proj_key,Batch_Key,Box_number,Policy_number,Scanned_user,scanned_dttm)" +
...
//		}
		public bool UpdateSrl(ArrayList arr)
        {
            ...
            sqlCmd.Transaction=sqlTrans;
                ...
                {
                    tmp = i+1;
                    if (arr[i].ToString().Trim().Length > wfePolicy._LENGTH_OF_PAGE_NAME)
                    {
                        sqlStr = "Update image_master set serial_no = " + tmp + " where proj_key=" + ctrlPolicy.ProjectKey + " and batch_key=" + ctrlPolicy.BatchKey + " and box_number = " + ctrlPolicy.BoxNumber + " and policy_number = " + ctrlPolicy.PolicyNumber + " and page_name = '" + arr[i].ToString().Substring(0,wfePolicy._LENGTH_OF_PAGE_NAME) + "'";
                    }
                    else
                    {
