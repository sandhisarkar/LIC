...
public class Twain
    ...
    public bool Acquire(bool prmShowUI, bool prmColorMode)
    {
        TwRC rc;
        TwCapability cap;
        CloseSrc();
        ...
        if (rc != TwRC.Success)
       	...
       	TwCapability capUnit = new TwCapability(TwCap.CAP_FEEDERENABLED, 1, TwType.Int16);
...
//        }
        ...
        cap = new TwCapability(TwCap.XResolution, 200, TwType.Fix32);
        ...
        cap = new TwCapability(TwCap.YResolution, 200, TwType.Fix32);
        ...
        {
            cap = new TwCapability(TwCap.ICAP_PIXELTYPE, (short)TwICap.TWPT_RGB, TwType.Int16);
            rc = DScap(appid, srcds, TwDG.Control, TwDAT.Capability, TwMSG.Set, cap);
            if (rc != TwRC.Success)
            {
                CloseSrc();
        ...
        {
            cap = new TwCapability(TwCap.ICAP_PIXELTYPE, (short)TwICap.TWPT_BW, TwType.UInt16);
            rc = DScap(appid, srcds, TwDG.Control, TwDAT.Capability, TwMSG.Set, cap);
            if (rc != TwRC.Success)
            {
                CloseSrc();
        ...
        ////Duplex
        cap = new TwCapability(TwCap.CAP_DUPLEXENABLED, (short)1, TwType.Int16);
        rc = DScap(appid, srcds, TwDG.Control, TwDAT.Capability, TwMSG.Set, cap);
        if (rc != TwRC.Success)
        {
            CloseSrc();
    ...
    public bool AcquireFixed(bool prmShowUI, bool prmColorMode, int prmPages, int prmDuplex)
    {
        ...
        if (prmColorMode == true)
        {
            cap = new TwCapability(TwCap.ICAP_PIXELTYPE, (short)TwICap.TWPT_RGB, TwType.UInt16);
            rc = DScap(appid, srcds, TwDG.Control, TwDAT.Capability, TwMSG.Set, cap);
            if (rc != TwRC.Success)
            {
                CloseSrc();
        ...
        {
            cap = new TwCapability(TwCap.ICAP_PIXELTYPE, (short)TwICap.TWPT_BW, TwType.Int16);
            rc = DScap(appid, srcds, TwDG.Control, TwDAT.Capability, TwMSG.Set, cap);
            if (rc != TwRC.Success)
            {
                CloseSrc();
        ...
        ///Duplex checking
        cap = new TwCapability(TwCap.CAP_DUPLEX);
        ...
        if (prmDuplex == ScanMode.SCAN_IN_DUPLEX_MODE)
        {
            ...
            {
                //Duplex negotiation
                cap = new TwCapability(TwCap.CAP_DUPLEXENABLED, (short)prmDuplex, TwType.Int16);
                rc = DScap(appid, srcds, TwDG.Control, TwDAT.Capability, TwMSG.Set, cap);
                if (rc != TwRC.Success)
                {
                    CloseSrc();
        ...
        else
        {
            ...
            {
                //Simplex negotiation
                cap = new TwCapability(TwCap.CAP_DUPLEXENABLED, (short)ScanMode.SCAN_IN_SIMPLEX_MODE, TwType.Int16);
                rc = DScap(appid, srcds, TwDG.Control, TwDAT.Capability, TwMSG.Set, cap);
                if (rc != TwRC.Success)
                {
                    CloseSrc();
                ...
                else
                {
                    ...
                    */
                    if (pxfr.Count == 0)
                    {
                        ...
                        do
                        {
                            cap = new TwCapability(TwCap.CAP_FEEDERLOADED);
                            rc = DScap(appid, srcds, TwDG.Control, TwDAT.Capability, TwMSG.Get, cap);
	...
	public TwainCommand PassMessage( ref Message m )
        ...
        {
            System.Diagnostics.Debug.Print(rc.ToString() + ", " + evtmsg.Message.ToString());
            
            return TwainCommand.Not;
        }
        if (evtmsg.Message == (short)TwMSG.XFerReady)
        {
            System.Diagnostics.Debug.Print("T ready lib");
        ...
        {
            System.Diagnostics.Debug.Print("here lib");
