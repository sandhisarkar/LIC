...
namespace Utils
{
	...
	public class BoxCreator: wItemCreator
	{
		...
		public override bool CreateBox(Credentials prmCrd,ControlInfo prmInfo,int prmBoxStatus,int prmPolicyStatus)
		{
			ds=new DataSet();
			...
			if(DuplicatePolicyCheck(ds,prmInfo)==false)
			{
				...
				{
					if (writeDb.SaveData(ds,prmCrd,prmInfo,nt,prmBoxStatus,prmPolicyStatus)==true)
					{
						return true;
					}
					else
		...
		private bool DuplicatePolicyCheck(DataSet prmDs,ControlInfo pControl)
		{
			...
			{
                sqlStr = "select policy_number from policy_master where proj_key=" + pControl.proj_Key + " and batch_key=" + pControl.batch_Key + " and box_number=" + Convert.ToInt32(prmDs.Tables[0].Rows[i]["CartonNo"].ToString()) + " and policy_number=" + prmDs.Tables[0].Rows[i]["PolicyNo"];
				cmd=new OdbcCommand(sqlStr,sqlCon);
				reader=cmd.ExecuteReader();
				
				if(reader.HasRows==true)
				{
                    System.Windows.Forms.MessageBox.Show("Duplicate policy found - " + prmDs.Tables[0].Rows[i]["PolicyNo"].ToString());
					return true;
				}
			}
			return false;
